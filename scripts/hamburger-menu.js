...
function setOpen(open){if(!menu||!openButton)return;menu.classList.toggle('hidden',narrow.matches&&!open);openButton.setAttribute('aria-expanded',String(open));}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&narrow.matches){setOpen(false);openButton?.focus();}});
